...
class PricesDatagridListener
{
    ...
    protected function addSelect(DatagridConfiguration $config)
    {
        ...
        $ormQuery
            ->addSelect(sprintf('GROUP_CONCAT(DISTINCT CONCAT_WS(\'|\', %1$s.value, %1$s.currency) SEPARATOR \';\') as defaultPrices', self::DEFAULT_JOIN_ALIAS))
            ->addSelect(sprintf('GROUP_CONCAT(DISTINCT CONCAT_WS(\'|\', %1$s.value, %1$s.currency) SEPARATOR \';\') as specialPrices', self::SPECIAL_JOIN_ALIAS))
            ->addSelect(sprintf('GROUP_CONCAT(DISTINCT CONCAT_WS(\'|\', %1$s.value, %1$s.currency) SEPARATOR \';\') as msrpPrices', self::MSRP_JOIN_ALIAS))
