...
class PurchaseOrderWorkflowCompletedListener
{
    ...
    public function onPurchaseOrderCompleted(InventoryUpdateEvent $event)
    {
        ...
        if (isset($data[$orderOnDemandKey])) {
            ...
            if ($order) {
                $workflowItem = $this->workflowManager->getWorkflowItem($order, 'marello_order_b2c_workflow_1');
                $currentStep = $workflowItem->getCurrentStep()->getName();
                if (in_array($currentStep, ['pick_and_pack', 'shipped'])) {
