...
class ShippingPriceCacheKeyTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAccessors()
    {
        self::assertPropertyAccessors(new ShippingPriceCacheKey(), [
            ['transport', $this->getEntity(UPSSettings::class, ['id' => 1])],
    ...
    public function testGenerateKey()
    {
        ...
        $key1->setTransport($this->getEntity(UPSSettings::class, ['id' => 1]));
        $this->assertKeysNotEquals($key1, $key2);
        $key2->setTransport($this->getEntity(UPSSettings::class, ['id' => 2]));
        $this->assertKeysNotEquals($key1, $key2);
        $key2->setTransport($this->getEntity(UPSSettings::class, ['id' => 1]));
        $this->assertKeysEquals($key1, $key2);
